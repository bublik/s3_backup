...
class BackupFu
  ...
  def initialize
    ...
    if File.exist?(File.join(RAILS_ROOT, 'config', 'amazon_s3.yml'))
  ...
  def prepare_db_for_restore
    puts "restore unimplemented for #{@db_conf[:adapter]}" && return unless (@db_conf[:adapter]) == 'postgresql'
  ...
  def restore_backup(key)
    puts "Restore not implemented for #{@db_conf[:adapter]}" unless @db_conf[:adapter] == 'postgresql'
    puts 'Restore not implemented for zip' if @fu_conf[:compressor] == 'zip'
  ...
  def db_filename_compressed
    db_filename.gsub('.sql', @fu_conf[:compressor].eql?('zip') ? '.zip' : '.tar')
  end
  ...
  def final_db_dump_path
    if(@fu_conf[:disable_compression])
      ...
      filename = db_filename.gsub('.sql', @fu_conf[:compressor].eql?('zip') ?'.zip' : '.tar.gz')
    end
    File.join(dump_base_path, filename)
  end

  def static_compressed_path    
    File.join(dump_base_path,
      "#{@fu_conf[:app_name]}_#{ @timestamp }_static.#{@fu_conf[:compressor].eql?('zip') ? 'zip' : 'tar'}")
  end

  def final_static_dump_path
    File.join(dump_base_path, 
      "#{@fu_conf[:app_name]}_#{ @timestamp }_static#{@fu_conf[:compressor].eql?('zip') ?'.zip' : '.tar.gz'}")
  end
